...
def render_path(render_poses, render_times, hwf, chunk, render_kwargs, gt_imgs=None, savedir=None,
    ...
    if savedir is not None:
        ...
        save_dir_unc = os.path.join(savedir, "uncertainty")
        os.makedirs(save_dir_unc, exist_ok=True)
    ...
    for i, (c2w, frame_time) in enumerate(zip(tqdm(render_poses), render_times)):
        rgb, disp, acc, extras = render(H, W, focal, chunk=chunk, c2w=c2w[:3,:4], frame_time=frame_time, **render_kwargs)
        rgbs.append(rgb.cpu().numpy())
        disps.append(disp.cpu().numpy())
        unc = extras['uncertainty_map']
        ...
        if savedir is not None:
            ...
            unc_np = uncs[-1]
            filename = os.path.join(save_dir_unc,'{:03d}.unc.npy'.format(i+i_offset))
            np.save(filename,unc_np)
            plt.figure(figsize=(6,6))
            plt.imshow(unc_np,cmap='jet')
            plt.colorbar()
            plt.axis('off')
            filename = os.path.join(save_dir_unc,'{:03d}.unc.png'.format(i+i_offset))
...
def create_nerf(args):
    ...
    if len(ckpts) > 0 and not args.no_reload:
        ...
        # remove mismatched output layer
        state_dict.pop('output_linear.weight', None)
        state_dict.pop('output_linear.bias', None)
        
        state_dict.pop('uncertainty_linear.weight', None)
        state_dict.pop('uncertainty_linear.bias', None)
        state_dict.pop('rgb_linear.weight', None)  # Optional: if RGB head changed
        state_dict.pop('rgb_linear.bias', None)    # Optional: if RGB head changed
        
        model.load_state_dict(state_dict, strict=False)
        
        if model_fine is not None:
           
            state_dict_fine = ckpt['network_fine_state_dict']
            state_dict_fine.pop('output_linear.weight', None)
            state_dict_fine.pop('output_linear.bias', None)
            state_dict_fine.pop('uncertainty_linear.weight', None)
            state_dict_fine.pop('uncertainty_linear.bias', None)
            state_dict_fine.pop('rgb_linear.weight', None)
            state_dict_fine.pop('rgb_linear.bias', None)
            model_fine.load_state_dict(state_dict_fine, strict=False)
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    #uncertainty_map = torch.max(log_var, dim=-1)[0]   #option A
   
    #weighted_log_var = log_var * weights                               #option B
    #uncertainty_map = torch.max(weighted_log_var, dim=-1)[0]

    #uncertainty_map = log_var[..., -1]                              #option C
    #uncertainty_map = torch.var(log_var, dim=-1)                    #option D 
    variance = torch.exp(torch.clamp(log_var, -10, 10))
    uncertainty_map = torch.sum(weights * variance,dim=-1)
    ...
    return rgb_map, disp_map, acc_map, weights, depth_map, uncertainty_map
...
def render_rays(ray_batch,
    ...
    if z_vals is None:
        ...
        if N_importance <= 0:
            ...
            rgb_map, disp_map, acc_map, weights, depth_map, uncertainty_map = raw2outputs(raw, z_vals, rays_d, raw_noise_std, white_bkgd, pytest=pytest)
            ...
            sample_unc = torch.exp(torch.clamp(raw[...,4], -10, 10))
            sample_unc = sample_unc[...,1:-1]
            sample_unc = sample_unc / (sample_unc.max(dim=-1, keepdim=True)[0]+ 1e-6)
            adaptive_weights = (weights[...,1:-1]*(1.0 + 0.10 * sample_unc))
            adaptive_weights = adaptive_weights + 1e-5
            z_samples = importance_sampling_ray(z_vals_mid,adaptive_weights,N_importance,det=(perturb==0.),pytest=pytest)
    ...
    ret = {'rgb_map' : rgb_map, 'disp_map' : disp_map, 'acc_map' : acc_map, 'uncertainty_map' : uncertainty_map, 'z_vals' : z_vals,
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        #Updated part for uncertainity
        img_loss = img2mse(rgb, target_s)

        variance = torch.clamp(
            extras['uncertainty_map'],
            min=1e-6,
            max=10.0
        )
        ...
        rgb_error = ((rgb - target_s)**2).mean(-1)
        
        uncertainty_loss = (
            rgb_error / (2.0 * variance)
        ...
        if i % 100 == 0:
            corr = torch.corrcoef(
                torch.stack([
                    (rgb - target_s).abs().mean(-1).flatten(),
                    sigma.flatten()
                ])
            )[0,1]
            ...
            print(
                f"Corr={corr.item():.3f}, "
                f"sigma_mean={sigma.mean().item():.3f}, "
                f"sigma_max={sigma.max().item():.3f}"
            )
        ...
        if depth_maps is not None:
            if args.depth_loss_weight > 1e-16:
                ...
                depth_loss = F.smooth_l1_loss(pred_depth, depth_s)
