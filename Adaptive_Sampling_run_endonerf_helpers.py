...
# Model (Modified DirectTemporalNerf)
class DirectTemporalNeRF(nn.Module):
    ...
    def __init__(
        ...
        self.hash_output_ch = self.hash_encoders[0].out_dim

        ####################################################################
        # Temporal Feature Blending
        ####################################################################

        self.temporal_feature_blender = nn.Sequential(

            nn.Linear(4, 64),   # xyz + time
            ...
            nn.Linear(64, len(self.hash_encoders))
        )
    ...
    def create_time_net(self):
        ...
        layers = [
            nn.Linear(
                self.input_ch + self.input_ch_time,
                self.W
            )
        ]
    ...
    def forward(self, x, ts):
        ...
        ####################################################################
        # Ensemble Hash Encoding
        ####################################################################

        features = []

        for encoder in self.hash_encoders:

            features.append(
                encoder(canonical_pts)
            )
        ...
        beta = torch.softmax(
            self.temporal_feature_blender(blend_input),
            dim=-1
        )
...
class NeRFOriginal(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, input_ch_time=1, output_ch=4, skips=[4],
        ...
        if use_viewdirs:
            ...
            nn.init.constant_(self.uncertainty_linear.bias, -1.0)
    ...
    def forward(self, x, ts):
        ...
        if self.use_viewdirs:
            ...
            log_var = self.uncertainty_linear(h)
            outputs = torch.cat([rgb, alpha, log_var], -1)
