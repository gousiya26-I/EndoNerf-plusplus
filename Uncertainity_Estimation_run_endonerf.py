...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    uncertainty_map = torch.sum(weights ** 2, dim=-1)
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        log_var = extras['uncertainty_map']
        log_var = torch.clamp(log_var, min=-6.0, max=3.0)
        var = torch.exp(log_var)
        sigma = torch.exp(0.5 * log_var)  # for monitoring only
        corr = torch.corrcoef(torch.stack([(rgb - target_s).abs().mean(-1).flatten(),sigma.flatten()]))[0, 1]
        
        if i % 100 == 0:
            print("Error-Sigma Corr:", corr.item())
        ...
        if torch.isnan(target_s).any():
            print("NaN in target")
        
        if i % 10 == 0:
            print(
                "sigma mean:", sigma.mean().item(),
                "sigma std :", sigma.std().item(),
                "sigma max:", sigma.max().item(),
